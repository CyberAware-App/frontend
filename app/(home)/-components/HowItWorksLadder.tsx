	...
	return (
		<ForWithWrapper
			...
			renderItem={(step, index) => {
				...
				const isStepCountEven = stepCount % 2 === 0;
					...
					<li key={step.title} className="contents">
						<p
							className={cnJoin(
								...
								isStepCountEven && "justify-self-start [grid-area:var(--grid-area)]"
							)}
							style={{ "--grid-area": `${stepCount}/3` } as React.CSSProperties}
						>
							{stepCount}
						</p>

						<hr className="absolute -z-1 h-[110%] w-full max-w-1 justify-self-center bg-white" />
						<span className="size-11 rounded-full bg-cyberaware-unizik-orange" />

						<div
							className={cnJoin(
								"relative isolate flex w-[150px] flex-col gap-2.5 bg-white p-4",
								isStepCountEven && "rotate-y-180 [grid-area:var(--grid-area)]"
							)}
							style={{ "--grid-area": `${stepCount}/1` } as React.CSSProperties}
						>
							<Triangle
								className={cnJoin("absolute top-1/2 -left-5 -z-1 size-[46px] -translate-y-1/2")}
							/>
							<h4
								className={cnJoin("text-cyberaware-aeces-blue", isStepCountEven && "rotate-y-180")}
							>
								{step.title}
							</h4>
							<p
